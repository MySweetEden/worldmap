  ...
  const handleButtonClick = (param: string) => {
    const paramsDict = params
      ? params.split(",").reduce((acc: { [key: string]: string }, keyValue) => {
        ...
        return acc;}, {})
      : {};
    paramsDict[param] = paramsDict[param] ? (parseInt(paramsDict[param]) + 1).toString() : "1";
    const updatedParams = Object.entries(paramsDict).map(([key, value]) => `${key}=${value}`).join(',');
    navigate(`/${updatedParams}`);
  };
  const clickedParams = params ? params.split(",") : [];
  const clickedNumParams = clickedParams ? clickedParams.map(param => param.split("=")) : [];
  console.log(clickedNumParams)
      ...
      {/* {countryNames
        .map(buttonName => {
          return (
            <button
              onClick={() => {
                handleButtonClick(buttonName[0]);
                console.log(clickedParams)
              }}
              // disabled={clickedNumParams.map(item=>item[0]).includes(buttonName[0])}
              key={buttonName[0]}
            >
            {buttonName[1]}
            </button>
